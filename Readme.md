# Transaction Workflow Engine — Home Assignment
...
```text
sqlink-task/
├── TransactionWorkflow_ApproachA.zip    # Vanilla .NET Clean Architecture
├── TransactionWorkflow_ApproachB.zip    # DNA-Infused Multi-Tenant Engine
├── TransactionWorkflow_ApproachD.zip    # Strategic Hybrid
├── buildAll.ps1                         # One-click extract + build all 3
├── testAll.ps1                          # Full integration test suite v4 (45 tests, keyvault-enabled)
...
│
├── FINAL_STATE.md                       # Phase 1–12 completion tracking
└── TEST_REPORT.md                       # Auto-generated by testAll.ps1 (45 tests)
```

---

## 🚀 Quick Start — Build & Run All 3
...
# Full integration tests — all 3 approaches via Docker (from root)
.\testAll.ps1
```
...
The sweet spot. Looks like a standard .NET project on the outside, but uses data-driven patterns on the inside. 80% of B's benefits with 20% of the complexity. Has the JSON `Rules` column and `DataProcessResult`, but lacks B's `EntityType` scoping — meaning it handles one workflow type per deployment.
...
## 🛠️ Skill Set Integration

Each approach maps to the skill library from previous implementations at a different depth:

| Skill | Purpose | A | B | D |
|-----------|---------|---|---|---|
| **Skill 01** — Core Interfaces / DataProcessResult | Structured errors, no exceptions | ❌ Exceptions | ✅ Full | ✅ Full |
| **Skill 02** — Object Processor / JSON Rules | Dynamic business rules as data | ❌ | ✅ Full | ✅ Full |
| **Skill 05** — Database Fabric | Generic data access patterns | ✅ EF Core | ✅ EF Core + JSON | ✅ EF Core + JSON |
| **Skill 08** — Flow Definition | Workflow as data, not code | ✅ DB rows | ✅ EntityType-scoped | ✅ DB rows |
| **Skill 09** — Flow Orchestrator | State machine engine | ✅ Specific | ✅ Generic (multi-tenant) | ✅ Generic (single-tenant) |
| **Skill 15** — API Gateway | HTTP ↔ domain mapping | ✅ Middleware | ✅ ResultMapper | ✅ ResultMapper |
| **Skill 29** — Testing | Automated quality assurance | ✅ xUnit | ✅ xUnit + isolation | ✅ xUnit |
| **Skill 45** — Design Patterns | Decorator, Adapter, Strategy | ✅ Decorator | ✅ All three | ✅ All three |

### The Core Philosophy — "Data, Not Code"
...
- **FREEDOM** = configuration that business users control (statuses, transitions, rules — all in the database)

Approach A puts everything in the MACHINE. Approach B maximizes FREEDOM. Approach D finds the strategic balance.

---

## 📡 API Endpoints (All Approaches)
...
### Bonus — Admin Workflow Management
...
| [`BugInvestigation_SKILL.md`](BugInvestigation_SKILL.md) | EF Core tracking conflict debugging methodology — 5-layer investigation framework |
| [`FINAL_STATE.md`](FINAL_STATE.md) | Phase 1–12 completion tracking: infrastructure, validation, error standards, security, bug fixes, reviewer feedback, internal keyvault |
| [`TEST_REPORT.md`](TEST_REPORT.md) | Auto-generated test results from `testAll.ps1` (45 tests across 3 approaches — 15 each) |

### Phase Progression (Phases 1–12)

```
Phase 1: DevOps Foundation          Phase 2: Input Validation       Phase 3: API Quality
  ├─ Docker healthcheck               ├─ FluentValidation              ├─ RFC 7807 ProblemDetails
  ├─ .env secrets extraction          └─ Fix B duplicate var bug       └─ Mermaid visualization
  └─ start.sh one-command launch
          │                                     │                              │
          ▼                                     ▼                              ▼
Phase 4: Documentation              Phase 5: Engine Purity          Phase 6: API Resilience
  └─ skill-mapped READMEs               ├─ D: Remove ITransactionRepo    ├─ B,D: GlobalExceptionMiddleware
                                       │   from WorkflowEngine          ├─ A: ProblemDetails upgrade
                                       └─ D: Pass priorTransition       └─ All: /health endpoint
                                            Count from service
          │                                     │                              │
          ▼                                     ▼                              ▼
Phase 7: Config Cleanup             Phase 8: Skill Integration      Phase 9: Bug Fixes (Critical)
  ├─ Remove hardcoded passwords        ├─ AI configs (Copilot,          ├─ AsNoTracking on cached queries
  ├─ .env.example for onboarding       │   Claude Code)                 ├─ FK-only navigation assignment
  └─ Fix Infrastructure csproj         └─ Skill maps + architecture     └─ Entity reload after mutation
                                            docs
          │                                     │                              │
          ▼                                     ▼                              ▼
Phase 10: Security Hardening        Phase 11: Reviewer Fixes        Phase 12: Internal Keyvault
  ├─ API Key Auth + dev-bypass         ├─ Concurrency/Idempotency docs   ├─ Docker Secrets integration
  ├─ Authorize + AdminOnly policy      ├─ Rule evaluator safety docs     ├─ Password removed from
  ├─ CORS configuration               ├─ CTO-choice framing fix         │   appsettings.json + env vars
  ├─ Env-aware exception detail        └─ Workflow diagram fix           ├─ AddKeyPerFile reads
  └─ XML docs in Swagger                                                │   /run/secrets/db_password
                                                                        ├─ SqlConnectionStringBuilder
                                                                        │   injects password at runtime
                                                                        └─ testAll.ps1 v4 provisions
                                                                            secret to %TEMP%
                                                                                    │
                                                                                    ▼
...
> **Note:** `testAll.ps1` v4 provisions an internal keyvault (Docker Secret) before building each approach. All fixes from previous versions are baked into source — no runtime patching needed. The `buildAll.ps1` only handles compilation.
...
## 🧪 testAll.ps1 — Full Integration Test Suite (v4)

Runs Docker-based end-to-end tests against all 3 approaches sequentially. For each approach it extracts fresh from zip, provisions the internal keyvault (Docker Secret), spins up containers, waits for the API, runs 15 tests, tears down, and moves to the next.
...
### Internal Keyvault (Docker Secrets)

The test script provisions a secure secret file before each approach's Docker build:

```
testAll.ps1 writes password to %TEMP%\txn_workflow_db_password.txt
  → docker-compose mounts it as Docker Secret
  → Container sees it at /run/secrets/db_password (tmpfs, RAM only)
  → Program.cs reads via AddKeyPerFile("/run/secrets")
  → SqlConnectionStringBuilder injects password into connection string
  → Password never appears in appsettings.json or API environment variables
```

The secret file is written to `%TEMP%` (not the project directory) to avoid antivirus false positives. It is cleaned up after each approach's test run.
...
| T5 | Available Transitions | Happy Path | CREATED → VALIDATED is listed |
...
| T8 | Transition to COMPLETED | Happy Path | Full workflow CREATED → COMPLETED |
| T9 | Transaction History | Happy Path | At least 3 history entries after full workflow |
| T10 | Bad Input → 400 | Error Path | Negative amount + invalid currency rejected |
| T11 | Invalid Transition → 400 | Error Path | CREATED → COMPLETED skipped, returns 400 |
| T12 | Not Found → 404 | Error Path | Non-existent transaction ID returns 404 |
...
```
Extract zip → Provision keyvault (secret to %TEMP%) → docker-compose up
→ Wait for API (90s max) → Run 15 API tests → docker-compose down → Cleanup secret → Next approach
```
...
Each approach shipped here solves the assignment. The extensions below are **not implemented** — they are architectural suggestions showing how the skill set patterns would scale to real-world requirements. They demonstrate **why** the pattern choices matter: what costs one line in Approach B costs a full rebuild in Approach A.

> **Execution strategy:** The `agent.md` file contains ready-to-use AI prompts (for Copilot, Claude, Cursor) that implement each extension following skill set patterns. The `.ai-config/v17-skill-library.md` file serves as the "rule book" — upload it before any AI session to enforce correct patterns.
...
**Pattern:** Skill 05 (Database Fabric) — add a `Dictionary<string, object> Data` property stored as a JSON column via EF Core `ValueConversion`. No schema changes needed per field.
...
**Pattern:** Skill 08 (Flow Definition) — add a nullable `ProjectId` foreign key to `WorkflowStatus` and `WorkflowTransition`. Project-specific transitions override global defaults. The repository implements fallback logic: look for project-specific transitions first, fall back to global if none found.
...
**Pattern:** Skill 15 (API Gateway) — auth configuration lives in `appsettings.json`, not in code. A Strategy Pattern reads `Authority`, `ClientId`, `Audience` from config and wires up `JwtBearer` automatically.
...
| Skill Library | `.ai-config/v17-skill-library.md` | skill → actual file mapping |
...
I deliberately built three implementations of the same assignment — not to pad the submission, but because **the right architecture depends on context that only the hiring team has:** team composition, product roadmap, and what features are coming next. Rather than guessing which one fits, I built all three as working proof-of-concepts so a CTO can evaluate the tradeoffs with code, not slides.

### The Approach — Skills-Driven Vibe Coding

Each approach was built using a **skills directory** — a library of documented patterns distilled from projects I've implemented in the past. The idea is straightforward: invest upfront in thorough documentation and reusable skill definitions so that architectural decisions can be made by reading docs, not digging into code.

This has a practical payoff beyond the assignment itself:

- **Decision-making without code review.** The documentation map, comparison tables, and interactive dashboards are designed to give a technical lead the full picture in minutes — which approach fits, what the tradeoffs are, and what changes later.
- **Continuity for future teams.** The `.ai-config/` folders, `CLAUDE.md`, and `copilot-instructions.md` files inside each approach mean that any developer — using any AI-assisted coding platform (Copilot, Claude Code, Cursor, or others) — can pick up the project and continue building with the same patterns. The skills directory acts as institutional knowledge that travels with the repo.
- **Vibe coding at scale.** The prompts in the AI config files aren't generic — they encode the specific architectural rules (DataProcessResult over exceptions, JSON rules over hardcoded logic, FK-only assignments) so AI assistants generate code that fits the existing codebase rather than defaulting to textbook patterns.
...
### Which Approach Fits? — It Depends

**Approach D** is the middle ground — it meets every requirement with a standard .NET stack and includes enough abstraction (DataProcessResult, JSON Rules) to show how the patterns scale, without unnecessary complexity.

**Approach B** shows what happens when you design for a platform from day one — the `EntityType` discriminator makes adding new workflow types a SQL-only operation. Whether that's worth the upfront investment depends entirely on the roadmap.
...
The key observation is that **the cost curves cross.** A is cheapest at month 1 but most expensive by month 12. B is most expensive at month 1 but cheapest once scope expands. D sits in between — a hedge that delays the B decision without the A penalty.
...
| Security audit flags plain-text secrets | 🟢 V4: Resolved (Docker Secrets + AddKeyPerFile) | 🟢 V4: Resolved | 🟢 V4: Resolved |
...
Architecture isn't just about code — it's also about the team maintaining it:
...
1. **Why three approaches instead of one?** There's no single "right" answer — the best choice depends on team composition, the product roadmap, and what features come next. All three are included as working proof-of-concepts so a CTO can evaluate the tradeoffs with a full picture rather than a single opinion.

2. **Scaling considerations at 10× volume.** The main pressure point is cache invalidation under concurrent transitions. `IMemoryCache` is singleton while `DbContext` is scoped — at high volume the tracking conflict window grows. A natural next step would be Redis with pub/sub invalidation and RowVersion-based optimistic concurrency.

3. **The EF tracking bug and how it was resolved.** 7 test failures across 3 approaches traced to 2 root causes, documented as a reusable investigation methodology in [`BugInvestigation_SKILL.md`](BugInvestigation_SKILL.md) — covering the DI Lifetime × Caching matrix and a decision tree for diagnosing 500 errors.
